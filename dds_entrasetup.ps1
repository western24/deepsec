...
# It configures:
# - App registration
# - App ID URI: api://<application-id>
# - Delegated scope: session:scope:connect
# - Optional ID token claim: upn
# - Microsoft Graph delegated permission required by the upn optional claim: profile
# - requestedAccessTokenVersion: 2
# - Deep Data Security app roles: EMPLOYEE, MANAGER
# - Demo users and app role assignments: emma -> EMPLOYEE, marvin -> MANAGER
...
param(
    ...
    [string]$SetupLogPath = ".\dds_entrasetup.log"
)
...
# Step 2: Create or reuse the app registration. The AppId becomes both the
# application_id and the default App ID URI used by Oracle Database.
...
# as an ID token optional claim. Oracle uses the App ID URI and token v2.
...
if (-not $SkipDeepDataSecuritySettings) {
    ...
    # Step 4: Create or update DDS app roles on the application registration.
    ...
    # Step 5: Add the app's own app roles as required permissions and add the
    # Microsoft Graph delegated profile permission required by the upn claim UI.
    ...
    # Step 6: Ensure the enterprise application exists and appears in the
    ...
    # Step 7: Grant each app role to the service principal itself. This mirrors
    # the portal consent state for the application permissions shown on the app.
    foreach ($appRole in $ddsAppRoles) {
        $appRoleId = Get-GraphProperty `
            -InputObject $appRole `
            -Name "id" `
            -Required
        $appRoleValue = Get-GraphProperty `
            -InputObject $appRole `
            -Name "value" `
            -Required
        ...
        Add-EntraAppRoleAssignment `
            ...
            -PrincipalObjectId $servicePrincipalObjectIdForAssignments `
            -ResourceServicePrincipalId $servicePrincipalObjectIdForAssignments `
            -AppRoleId $appRoleId `
            -RoleValue $appRoleValue

        $ddsAssignments.Add("servicePrincipal:$($servicePrincipalObjectIdForAssignments):$appRoleValue")
    }
...
$envContent = @"
TENANT_ID=$TenantId
TENANT_PRIMARY_DOMAIN=$tenantPrimaryDomain
ORACLE_DB_APP_NAME=$DisplayName
ORACLE_DB_APP_ID=$($application.AppId)
ORACLE_DB_APP_OBJECT_ID=$($application.Id)
ORACLE_DB_APP_ID_URI=$appIdUri
ORACLE_DB_SCOPE=$appIdUri/$ScopeName
ORACLE_DB_SCOPE_NAME=$ScopeName
ORACLE_DB_SCOPE_ID=$($oracleScope.id)
ORACLE_DB_REQUESTED_ACCESS_TOKEN_VERSION=2
ORACLE_DB_OPTIONAL_ID_TOKEN_CLAIM=upn
ORACLE_DB_OPTIONAL_CLAIM_MICROSOFT_GRAPH_DELEGATED_SCOPES=$optionalClaimMicrosoftGraphDelegatedScopeSummary
ORACLE_IDENTITY_PROVIDER_CONFIG=$oracleIdentityProviderConfig
SETUP_LOG_PATH=$SetupLogPath
DDS_SERVICE_PRINCIPAL_OBJECT_ID=$servicePrincipalObjectId
DDS_APP_ROLES=$ddsAppRoleSummary
DDS_EMPLOYEE_USER_UPN=$employeeUserPrincipalNameOutput
DDS_EMPLOYEE_USER_OBJECT_ID=$employeeUserObjectId
DDS_EMPLOYEE_USER_CREATED=$employeeUserCreated
DDS_EMPLOYEE_USER_TEMPORARY_PASSWORD=$employeeTemporaryPassword
DDS_EMPLOYEE_ROLE=$employeeRoleAssigned
DDS_MANAGER_USER_UPN=$managerUserPrincipalNameOutput
DDS_MANAGER_USER_OBJECT_ID=$managerUserObjectId
DDS_MANAGER_USER_CREATED=$managerUserCreated
DDS_MANAGER_USER_TEMPORARY_PASSWORD=$managerTemporaryPassword
DDS_MANAGER_ROLE=$managerRoleAssigned
DDS_ASSIGNMENTS=$ddsAssignmentSummary
"@
